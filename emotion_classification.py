...
# Using for GPU
def config_gpu(using_config=True, gpu='0'):
    if using_config:
...
'''
Code check and statistical data and split train/val/test
'''
path_csv = './dataset/fer2013/fer2013.csv'  # Replace by your dataset
...
"""
I used two callbacks one is `early stopping` for avoiding over-fitting training data
...
'''
I tried both `Nadam` and `Adam`, the difference in results is not different but I finally went with Nadam as it is more 
popular. Training/save model
'''
...
with open("weight/model.yaml", "w") as yaml_file:
...
pyplot.savefig('./results/epoch_history_dcnn.png')
pyplot.show()
...
pyplot.savefig('./results/performance_dist.png')
pyplot.show()
...
pyplot.savefig("./results/confusion_matrix_dcnn.png")
